...
class AbstractRecognizer():
    def __init__(self, config: Config, commands: dict[str, dict], 
        ...
        #         """\n**IMPORTANT**: You CANNOT CHAT or TALK DIRECTLY with the user, even if they ask it. Just GENERATE THE GRAPH."""
        ...
        #         """\nTo execute a command, some previous command MUST execute it (except the command with ID 1)."""
        # )
...
class ComplexRecognizer(AbstractRecognizer):
    def __init__(self, config: Config, commands: dict[str, dict], 
        ...
        """
        if config.chat_model in ["gpt-4o", "o1-preview", "o1-mini"]:
            recognition_messages = [
                {
                    'role': config.base_message_role,
                    'content':
                    """You are a tool that, based on the user's prompt, detects the series of commands that must be executed, arguments that each command will have, and the relationship between each command (for example, what data generated by a command will be used as an argument for another)."""
                    """\nDO NOT write '```plaintext' or similar to denote format. JUST WRITE THE GRAPH, NOTHING ELSE."""
                    """\n**IMPORTANT**: You CANNOT CHAT or TALK DIRECTLY with the user, even if they ask it. Just GENERATE THE GRAPH."""
                    """\n*IMPORTANT*: *WRITE in the LANGUAGE that the USER writes his/her prompt in*."""
                    """\n*IMPORTANT*: You can ONLY use the given commands. **NEVER try to use OTHERS**."""
                    """\n*IMPORTANT*: While creating the graph, you are NOT talking to the user. You are JUST CREATING THE GRAPH, so do not write suggestions for a user inside of the graph. Create the complete graph by your own."""
                    """\n\nDepending on the prompt, you will use different commands and different arguments and relationships between commands. The only way you can see data generated by other commands from a command is by passing them as arguments."""
                    """\n\nDouble quotes (\"\") that are not part of defining a string (such as writing a quote inside a string) must be either escaped with a backslash or just use single quotes ('')."""
                    ...
                    """\n\n## User"""
                    """\n*IMPORTANT*: The user might write incorrectly and their inputs might be ambiguous."""
                    """\nProvide all the relevant context in the arguments of the commands, so that you're not ambiguous."""
                    """\nThe graph MUST perform the instruction COMPLETELY including minor details."""

                    """\n\nDon't be redundant."""

                    """\n\nGraphs examples (with some custom commands that you might not have):"""
                    ...
                    """\n\nExample 4:"""
                    """\nUser prompt: 'Write an article of two paragraphs. If I like it, write one more paragraph and save the whole article.'"""
                    """\nYour response might be: '[1, "THINK", {"about": "Two-paragraph article"}, [[2, null, null]]]\n[2, "WRITE_TO_USER", {"content": "__&1.thought__"}, [[3, null, null]]]\n[3, "REQUEST_USER_INPUT", {"message": "Do you like the article? (yes or no)"}, [[4, null, null]]]\n[4, "IF", {"condition": "__&3.input__ == 'yes'"}, [[5, "result", 1], [6, "result", 0]]]\n[5, "THINK", {"about": "One additional paragraph for article: __&1.thought__"}, [[7, null, null]]]\n[6, "WRITE_TO_USER", {"content": "Alright, not saving it then."}, []]\n[7, "CONCATENATE_STRINGS", {"str1": "__&1.thought__", "str2": "__&5.thought__", "sep": "\\n"}, [[8, null, null]]]\n[8, "WRITE_FILE", {"content": "__&7.concatenated__", "file_path": "three_paragraph_article.txt"}, []]'"""

                    """\n\nExample 5:"""
                    """\nUser prompt: 'Write an article of two paragraphs. If I like it, write one more paragraph.'"""
                    """\nYou will NOT ask the user to write the paragraphs. You'll write them and then ask the user if he/she liked the article."""

                    """\n\nExample 6:"""
                    """\nUser prompt: 'Write a two paragraph article.'"""
                    """\nYour response might be: '[1, "THINK", {"about": "Two-paragraph article"}, [[2, null, null]]]\n[2, "WRITE_TO_USER", {"content": "__&1.thought__"}, []]'"""
                }
            ]

        elif config.chat_model in ["gpt-4", "gpt-4-turbo"]:
            recognition_messages = [
                {
                    'role': config.base_message_role,
                    'content':
        ...
        elif config.chat_model in ["gpt-3.5-turbo"]:
            recognition_messages = [
                {
                'role': config.base_message_role,
                'content':
                    ...
                    """\n\n## Data reference"""
                    """\n*IMPORTANT*: The generates_data field of the commands are NOT part of the graph structure. They're NOT arguments. They are only used in data referencing (__&i.data__)."""
                    """\nTo reference data generated by a command, use __&i.data__. 'i' is the ID of the command; 'data' is the name of the generated data."""
                    """\nAlways consider the DATA TYPE of the data you are referencing (e.g. string-types can not have indices like '__&i.thought[j]__'; data does not have attributes unless specified)."""
                    """\nYou can ONLY reference data in the arguments of the nodes."""
                    """\nThe only way to reference the data of other commands is by using the __&i.data__ referencing. Commands DO NOT KNOW each other's data."""
                    ...
                    """\n\n## User"""
                    """\nProvide ALL the relevant context in the arguments of the commands, so that you're not ambiguous."""
                    """\nEx., if you're writing additional paragraphs for a text, include the text in the arguments."""
                    ...
                    # # """\n\n*IMPORTANT*: The generates_data field of the commands are NOT part of the graph structure. They are only used in data referencing (__&i.data__)."""
                    # # """\n*IMPORTANT*: All nodes must have the correct and complete structure."""
                    # # """\n*IMPORTANT*: The structure is JSON-Lines, not JSON, so DO NOT WRAP the entire answer with square brackets. [[command1], [command2], ...] (JSON) -> [command1]\n[command2]\n... (JSON-Lines)"""
                    # """\nThe user is asking YOU (the SYSTEM) to do something (ex. 'Write an article' is requesting the system to write an article, so the SYSTEM will write the article, not the user)."""
                    # # """\nYou are interacting with a user, so DO NOT suppose/guess their answers."""
                    # """\nIf it's not necessary, DO NOT ask the user to input something. If they ask you to write something, write it yourself."""
                    # """\nThe graph of commands must fulfill the user's instruction COMPLETELY and precisely."""
                }
            ]
